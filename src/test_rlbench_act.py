...
from src.utils import RankedLogger, TemporalAgg, extras
...
def get_image_data(cfg, obs):
    all_cam_images = []
    for camera_name in cfg.data.train.cameras:
        image = getattr(obs, f"{camera_name}_rgb").astype(float)
        if cfg.data.train.include_depth:
            depth = getattr(obs, f"{camera_name}_depth").astype(float)
...
def convert_obs(cfg, obs, device):
    ...
    if "pcd" in cfg.data.train._target_.lower():
        ...
        image_data = get_image_data(cfg, obs)
        image_data = image_data.unsqueeze(0)
        data_dict["image"] = image_data.to(device)
...
def eval(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    model = RLBenchACTBCModule.load_from_checkpoint(
    ...
    for from_episode_number in range(cfg.episodes_num):
        ...
        temp_agg = TemporalAgg(
            apply=cfg.temporal_agg.apply,
            action_dim=cfg.temporal_agg.action_dim,
            chunk_size=cfg.temporal_agg.chunk_size,
            k=cfg.temporal_agg.k,
        )
        log.info(
        ...
        if cfg.offline:
            ...
            for step_id in range(cfg.max_steps):
                data_dict = convert_obs(cfg, obs, device)
                description_token = clip.tokenize(description[0]).to(device)
                task_goal = (
                    clip_model.encode_text(description_token).reshape(1, -1).float()
                )
                data_dict["goal_cond"] = task_goal

                pred_action = model(data_dict)["a_hat"][
                    0
                ].cpu()  # (chunk_size, action_dim)

                if step_id != 0 and cfg.temporal_agg.apply:
                    quat = pred_action[:, 3:7]
                    rot_6d = matrix_to_rotation_6d(quaternion_to_matrix(quat))
                    pred_action = torch.cat(
                        [pred_action[:, :3], rot_6d, pred_action[:, 7:]], dim=-1
                    )
                    pred_action = torch.from_numpy(
                        temp_agg(pred_action.numpy())
                    )  # (action_dim,)
                    assert len(pred_action.shape) == 1
                ...
                while i < max_retry:
                    try:
                        obs, reward, terminate = task.step(pred_action[i])
                ...
                if reward > 0:
                    break
            print("success num", success_rate)
...
@hydra.main(
    version_base="1.3", config_path="../configs", config_name="test_rlbench_act.yaml"
)
