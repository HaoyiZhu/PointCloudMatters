...
@register_env("PegInsertionSide-3steps-MultiView", max_episode_steps=200)
class PegInsertionSideEnv(StationaryManipulationEnv):
    ...
    def _register_cameras(self):
        ...
        cam_cfg.uid = "front_camera"
        back_pose = look_at([0, 0.6, 0.4], [-0.1, 0, 0.1])
        right_pose = look_at([0.3, 0, 0.3], [-0.1, 0, 0.1])
        left_pose = look_at([-0.3, 0, 0.3], [-0.1, 0, 0.1])
        ...
        for angle in angles:
            ...
            left_y = r * np.cos(0 - angle_rad)
            left_x = r * np.sin(0 - angle_rad)
            left_angle_pose = look_at([left_x, left_y, 0.2], [0, 0, 0.1])
            ...
            down_y = r * np.cos(0 - angle_rad)
            down_z = r * np.sin(0 - angle_rad)
            right_angle_pose = look_at([0, down_y, 0.2 - down_z], [0, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
        ...
        return [cam_cfg, back_camera, right_camera, left_camera] + additional_cameras
...
@register_env("PegInsertionSide-3steps-v1", max_episode_steps=200)
class PegInsertionSideEnvChangeCamera(PegInsertionSideEnv):
    _clearance = 0.003

    def _register_cameras(self):
        pose = look_at([0.5, -0.5, 0.8], [0.05, -0.1, 0.4])
        return CameraConfig(
...
@register_env("PegInsertionSide-3steps-v0", max_episode_steps=200)
class PegInsertionSideEnvChangeCamera(PegInsertionSideEnv):
    _clearance = 0.003

    def _register_cameras(self):
        pose = look_at([0, -0.3, 0.2], [0, 0, 0.1])
        return CameraConfig(
            "base_camera", pose.p, pose.q, 128, 128, np.pi / 2, 0.01, 10
        )
