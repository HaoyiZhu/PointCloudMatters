...
class TurnFaucetBaseEnv(BaseEnv):
    ...
    def _register_cameras(self):
        ...
        front_pose = pose
        back_pose = look_at([0.4, 0, 0.3], [0, 0, 0.1])
        right_pose = look_at([0, 0.4, 0.3], [0, 0, 0.1])
        left_pose = look_at([0, -0.4, 0.3], [0, 0, 0.1])
        front_camera = CameraConfig(
            "front_camera", front_pose.p, front_pose.q, 128, 128, np.pi / 2, 0.01, 10
        )
        ...
        for angle in angles:
            ...
            left_angle_pose = look_at([left_x, left_y, 0.3], [0, 0, 0.1])
            ...
            right_angle_pose = look_at([right_x, right_y, 0.3], [0, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
        ...
        for angle in angles:
            ...
            right_angle_pose = look_at([down_x, 0, 0.3 - down_z], [0, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
...
@register_env("TurnFaucet-MultiView", max_episode_steps=200, override=True)
class TurnFaucetEnv(TurnFaucetBaseEnv):
    ...
    def _initialize_task(self):
        ...
        self.lfinger_pcd = trimesh.sample.sample_surface(self.lfinger_mesh, 256)[0]
        self.rfinger_pcd = trimesh.sample.sample_surface(self.rfinger_mesh, 256)[0]
...
@register_env("TurnFaucet-light-base", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_Base(TurnFaucetEnv):
    def _register_cameras(self):
        pose = look_at([-0.4, 0, 0.3], [0, 0, 0.1])
...
@register_env("TurnFaucet-light-0.03", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_0_03(TurnFaucetEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.03)


@register_env("TurnFaucet-light-0.05", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_0_05(TurnFaucetEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.05)


@register_env("TurnFaucet-light-0.15", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_0_15(TurnFaucetEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.15)


@register_env("TurnFaucet-light-0.6", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_0_6(TurnFaucetEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.6)


@register_env("TurnFaucet-light-1.8", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_1_8(TurnFaucetEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(1.8)


@register_env("TurnFaucet-light-3.0", max_episode_steps=200, override=True)
class TurnFaucetEnv_Scene_3_0(TurnFaucetEnv_Scene_Base):
    def _setup_lighting(self):
