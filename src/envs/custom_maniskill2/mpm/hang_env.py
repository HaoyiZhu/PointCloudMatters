...
@register_env("Hang-MultiView", max_episode_steps=350)
class HangEnv(MPMBaseEnv):
    ...
    def _register_cameras(self):
        ...
        back_pose = look_at([-0.45, 0.1, 0.6], [-0.1, 0, 0.1])
        right_pose = look_at([0, 0.45, 0.6], [-0.1, 0, 0.1])
        left_pose = look_at([0, -0.45, 0.6], [-0.1, 0, 0.1])

        front_camera = CameraConfig("front_camera", p, q, 128, 128, np.pi / 2, 0.01, 10)
        back_camera = CameraConfig(
            "back_camera", back_pose.p, back_pose.q, 128, 128, np.pi / 2, 0.01, 10
        )
        right_camera = CameraConfig(
            "right_camera", right_pose.p, right_pose.q, 128, 128, np.pi / 2, 0.01, 10
        )
        left_camera = CameraConfig(
            "left_camera", left_pose.p, left_pose.q, 128, 128, np.pi / 2, 0.01, 10
        )

        r = 0.45
        angles = [5, 10, 30]  # 左右相机的角度差
        additional_cameras = []
        for angle in angles:
            angle_rad = np.radians(angle)
            # 计算左侧相机的位置
            left_x = r * np.cos(0 - angle_rad)
            left_y = r * np.sin(0 - angle_rad)
            left_angle_pose = look_at([left_x, left_y, 0.5], [-0.1, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
                    f"left_camera_{angle}",
                    left_angle_pose.p,
                    left_angle_pose.q,
                    128,
                    128,
            ...
            # 计算右侧相机的位置
            right_x = r * np.cos(0 + angle_rad)
            right_y = r * np.sin(0 + angle_rad)
            right_angle_pose = look_at([right_x, right_y, 0.5], [-0.1, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
                    f"right_camera_{angle}",
                    right_angle_pose.p,
                    right_angle_pose.q,
                    128,
                    128,
        ...
        for angle in angles:
            angle_rad = np.radians(angle)
            # up
            up_x = r * np.cos(0 + angle_rad)
            up_z = r * np.sin(0 + angle_rad)
            left_angle_pose = look_at([up_x, 0, 0.5 + up_z], [-0.1, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
                    f"up_camera_{angle}",
                    left_angle_pose.p,
            ...
            # down
            down_x = r * np.cos(0 - angle_rad)
            down_z = r * np.sin(0 - angle_rad)
            right_angle_pose = look_at([down_x, 0, 0.5 + down_z], [-0.1, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
                    f"down_camera_{angle}",
                    right_angle_pose.p,
                    right_angle_pose.q,
                    128,
                    128,
    ...
    def evaluate(self, **kwargs):
        ...
        return dict(
            success=(
                ...
                and down
                and bottom
                and len(np.where((particles_v < 0.5) & (particles_v > -0.5))[0])
...
@register_env("Hang-light-base", max_episode_steps=350, override=True)
class HangEnv_Scene_Base(HangEnv):
    def _register_cameras(self):
        p, q = [0.45, -0.0, 0.5], euler2quat(0, np.pi / 5, np.pi)
        base_camera = CameraConfig("base_camera", p, q, 128, 128, np.pi / 2, 0.01, 10)
...
@register_env("Hang-light-0.03", max_episode_steps=350, override=True)
class HangEnv_Scene_0_03(HangEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.03)


@register_env("Hang-light-0.05", max_episode_steps=350, override=True)
class HangEnv_Scene_0_05(HangEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.05)


@register_env("Hang-light-0.15", max_episode_steps=350, override=True)
class HangEnv_Scene_0_15(HangEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.15)


@register_env("Hang-light-0.6", max_episode_steps=350, override=True)
class HangEnv_Scene_0_6(HangEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(0.6)


@register_env("Hang-light-1.8", max_episode_steps=350, override=True)
class HangEnv_Scene_1_8(HangEnv_Scene_Base):
    def _setup_lighting(self):
        self.setup_scene(1.8)


@register_env("Hang-light-3.0", max_episode_steps=350, override=True)
class HangEnv_Scene_3_0(HangEnv_Scene_Base):
    def _setup_lighting(self):
...
@register_env("Hang-depth-noise-std0.01", max_episode_steps=350, override=True)
class HangEnv_Depth_Noise_STD001(HangEnv):
    def _register_cameras(self):
        p, q = [0.45, -0.0, 0.5], euler2quat(0, np.pi / 5, np.pi)
        front_camera = CameraConfig("base_camera", p, q, 128, 128, np.pi / 2, 0.01, 10)
        return [front_camera]

    def get_images(self):
        images = OrderedDict()
        for name, camera in self._cameras.items():
            rgbd_img = camera.get_images()
            with temp_seed(self._main_seed):
                noise = np.random.normal(
                    0, 0.01, size=rgbd_img["Position"][..., [2]].shape
                )
            rgbd_img["Position"][..., [2]] = rgbd_img["Position"][..., [2]] + noise
            images[name] = rgbd_img
        return images


@register_env("Hang-depth-noise-std0.05", max_episode_steps=350, override=True)
class HangEnv_Depth_Noise_STD005(HangEnv):
    def _register_cameras(self):
        p, q = [0.45, -0.0, 0.5], euler2quat(0, np.pi / 5, np.pi)
        front_camera = CameraConfig("base_camera", p, q, 128, 128, np.pi / 2, 0.01, 10)
        return [front_camera]
    ...
    def get_images(self):
        ...
        for name, camera in self._cameras.items():
            ...
            with temp_seed(self._main_seed):
                noise = np.random.normal(
                    0, 0.05, size=rgbd_img["Position"][..., [2]].shape
                )
            rgbd_img["Position"][..., [2]] = rgbd_img["Position"][..., [2]] + noise
            images[name] = rgbd_img
        return images
