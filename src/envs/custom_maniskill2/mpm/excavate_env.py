...
@register_env("Excavate-MultiView", max_episode_steps=250)
class ExcavateEnv(MPMBaseEnv):
    ...
    def _register_cameras(self):
        ...
        back_pose = look_at([0.2, -0.1, 0.4], [-0.1, 0, 0.1])
        right_pose = look_at([0, 0.2, 0.4], [-0.1, 0, 0.1])
        left_pose = look_at([0, -0.2, 0.4], [-0.1, 0, 0.1])
        ...
        # return [front_camera, back_camera, right_camera, left_camera]
        r = -0.2
        angles = [5, 10]  # 左右相机的角度差
        additional_cameras = []
        for angle in angles:
            ...
            left_angle_pose = look_at([left_x, left_y, 0.4], [-0.1, 0, 0.1])
            ...
            right_angle_pose = look_at([down_x, 0, 0.4 - down_z], [-0.1, 0, 0.1])
            additional_cameras.append(
                CameraConfig(
